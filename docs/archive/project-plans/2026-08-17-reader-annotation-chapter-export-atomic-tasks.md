...
- 完成日期：2026-08-17
- 提交：`2580612`
- 结果：新增 memoized `ReaderAnnotationsPanel`，提供全部、翻译、划线和批注四种筛选及分类计数；三类条目通过图标、文字、边框、底线和蓝/橙/绿语义样式共同区分，展示完整章节路径与页码、原文、正文和本地化时间。卡片及定位按钮支持跳转原文，卡片支持 Enter/Space 键盘操作；列表每批加载 80 条，活动条目会自动扩展到所在分页。面板关闭时不构建列表，稳定回调与卡片字段比较保证筛选不重绘阅读器、编辑或删除只更新受影响条目。
- 验证：针对性 20 项测试、`npm test`、`npm run lint`、`npm run build`、`git diff --check` 通过；应用内浏览器干净冷启动验证桌面与 430×850 窄屏书库可渲染、书库入口交互正常、无错误覆盖层和控制台告警。
- 风险/后续：普通浏览器无法访问 Electron 本地书籍数据库，因此真实 PDF/EPUB 批注栏的筛选、跨页跳转及深色阅读主题视觉仍需在 AT-15 的 Electron 集成验收中完成；分类、分页、键盘契约、稳定回调和阅读器单实例边界已有自动化覆盖。
...
- 完成日期：2026-08-17
- 提交：`fd6f0b8`
- 结果：扩展 `readerAnnotationSerializer`，可直接规范化数据库批注行，并以独立的 `outline_path_json` 为真实章节来源构建、排序和分组 `ExportAnnotationRecord`；每条记录保留类型、原文、正文、完整路径、页码、时间、状态及 `book:{id}#annotation:{annotationId}` 深链接。Markdown 使用 H1-H6 表达可用层级，超过标题深度后仍输出完整加粗路径，不截断任意深度目录；无目录记录归入“未识别章节”且保留页码。Notes 同步区使用书籍与批注稳定 ID 注释标记，重复同步只替换生成区，删除记录自动消失，标记区外手写内容原样保留，残缺标记会拒绝写入。导出每次重新查询数据库，不再读取当前章节，并提供局部“同步中”状态与防重复触发保护，不阻塞阅读器其他操作。
- 验证：针对性 28 项测试、`npm test`、`npm run lint`、`npx tsc -b --pretty false`、`npm run build`、`git diff --check` 通过；应用内浏览器干净冷启动验证书库主导航交互正常、无错误覆盖层和控制台告警。
- 风险/后续：普通浏览器没有 Electron 本地书籍与 Notes 数据库，真实同步按钮状态、Notes 写入和新深链接点击定位需在 AT-15 的桌面集成验收中复核；HTML/DOCX/PDF 样式和深链接解析由 AT-14 接续实现。CodeRabbit CLI 因当前 Windows 环境缺少 `sh` 且安装脚本 TLS 握手失败而未能执行，不影响已完成的本地门禁结果。
...
- 完成日期：2026-08-17
- 提交：`0144ed4`
- 结果：HTML 与 PDF 复用统一打印样式，翻译、划线和批注分别使用 T/U/N 图标、文字标签、蓝/橙/绿边框及背景；Markdown 在 H6 后生成继续嵌套的目录列表。Word 导出由伪 `.doc` HTML 升级为真实 `.docx` OOXML，保留 H1-H6、任意深度列表、三类语义样式和 `book:{id}#annotation:{annotationId}` 超链接；Word 原生 9 级列表之外使用递增缩进继续表达更深层级。DOCX 转换模块只在用户选择 Word 导出时动态加载，不进入普通启动、阅读或 PDF 渲染路径。Notes 深链接解析同时支持书籍、旧 `#Chapter` 和新 `#annotation:` 三种目标；书库先完成完整阅读器加载，再等待 PDF 页数或 EPUB 章节就绪并复用已有批注定位链。导出按钮使用局部 loading 与 `finally` 恢复，失败后可直接重试，既不清空也不改写 Notes；PDF 隐藏窗口在成功和异常路径均可靠销毁。
- 验证：针对性 30 项测试、`npm test`、`npm run lint`、`npx tsc -b --pretty false`、`npm run build`、`git diff --check` 通过。DOCX 测试验证标准内容类型、标题、15 层目录、三类标签/颜色和批注深链接关系；生产构建将 526 KB DOCX 适配器拆为独立动态块。`npm audit --omit=dev` 未发现本任务新增依赖漏洞；候选 `html-to-docx` 因已知高危 `image-size` DoS 链及旧版无效 OOXML 输出被移除。应用内浏览器验证冷启动及书库→笔记→书库交互正常，无框架错误覆盖层和控制台告警。
- 风险/后续：普通浏览器没有 Electron 数据与保存对话框，真实本地书籍上的 annotation 点击定位、PDF 打印和 Word 保存仍在 AT-15 桌面集成验收；430×850 窄屏下书库保留既有桌面最小内容宽度，右侧空态文案存在横向溢出，纳入 AT-15 体验检查。CodeRabbit CLI 仍因当前 Windows `curl` TLS 握手失败无法安装，未执行外部审查，所有本地门禁已完成。
