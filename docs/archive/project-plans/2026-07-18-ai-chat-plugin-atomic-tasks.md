...
| AT-24 | 国际化、键盘与无障碍收口 | E | AT-23 | 已完成 |
| AT-25 | 安全、集成、跨模块与打包验收 | E | AT-01 至 AT-24 | 已完成 |
...
- 国际化检查：新增完整 AI locale 递归契约测试，中英文键集合、非空值和所有 `{{token}}` 插值参数完全一致；运行状态播报、媒体替代文本和图片打开名称均提供中英文文案。
- 键盘检查：既有 Enter 发送、Shift+Enter 换行和 IME 保护继续通过；应用内浏览器在 800×600 下验证输入框保持焦点、Shift+Enter 产生换行并显示 2px 高对比焦点环。
- 对话框检查：工具授权与媒体查看器统一接入 AccessibleDialog，支持初始焦点、Tab/Shift+Tab 焦点循环、Escape 关闭和触发控件焦点恢复；焦点恢复延迟到 Portal 节点移除后的微任务执行。
- 流式播报检查：消息时间线和 AI 内容容器不再作为 live region；独立 sr-only 原子状态区只在 run ID 或运行状态变化后 240ms 播报，不依赖 token、消息数组或流式文本更新。
- 媒体检查：图片缩略图按钮有本地化可访问名称，缩略图、查看器原图和视频播放器在供应商未提供 alt 时使用本地化替代文本；媒体操作图标从可访问名称中隐藏。
- 主题检查：AI 控件统一使用主题 accent 派生的 2px `focus-visible`；成功、警告和失败状态由主题正文色混合变量派生，覆盖运行检查器、工具风险与工具终态，在四套主题下不依赖固定浅色背景。
- 最小窗口检查：800×600 隔离工作区显示会话列表、Agent 选择、输入区和单一原子 live region，页面宽度与 viewport 同为 800px，无横向溢出；测试 mock 未调用外部模型或写入用户数据。
- 针对性测试：新增 `aiLocales.test.ts` 与 `aiAccessibility.test.ts`，连同 AccessibleDialog、chat utils、locale 与工具箱契约测试共 19 项通过。
- 全量门禁：`npm test`、`npm run lint`、`npm run build`、`npm audit --audit-level=high` 与 `git diff --check` 全部通过；依赖审计为 0 个漏洞。
- 影响检查：AccessibleDialog 的新增 role、class 与遮罩关闭能力均为可选参数，供应商、Agent 和 MCP 既有对话框行为保持不变；AI 之外模块全量回归通过。
...
- 新增最终验证记录与需求—实现—验收矩阵，FR-001 至 FR-906 按领域映射到实现和自动化证据。
- 全量测试、lint、build、依赖审计和 diff 检查通过；安全专项覆盖凭据、SSRF、路径、协议、Markdown、MCP 权限、备份和恢复。
- macOS x64 原生模块验证通过，未签名目录包成功生成；本机无有效 Developer ID，签名与公证明确列为分发前置条件。
- Windows 实包校验因当前工作区缺少 Windows x64 构建产物而无法完成，已作为环境阻塞项记录，未误报为通过。
- 真实 Grok/xAI 凭据未提供；兼容协议 mock 端到端通过，真实账户限流、计费和长任务冒烟列为后补验收。
