...
export class AIVideoGenerationService {
  ...
  async run(input: {
    ...
    const assetId = Number(this.dependencies.db.prepare(`
      INSERT INTO ai_media_assets (provider_id, media_type, mime_type, status, created_at, updated_at)
      VALUES (?, 'video', 'video/mp4', 'queued', ?, ?)
    `).run(input.providerId, this.now().toISOString(), this.now().toISOString()).lastInsertRowid)
