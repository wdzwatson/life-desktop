...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getApplicationLogInfo: () => ipcRenderer.invoke('logs:getInfo'),
  openApplicationLogDirectory: () => ipcRenderer.invoke('logs:openDirectory'),
  exportApplicationLogs: () => ipcRenderer.invoke('logs:export'),
  ...
  onDouyinSyncDiagnostic: (callback: (data: any) => void) => {
    const subscription = (_event: any, data: any) => callback(data)
    ipcRenderer.on('video:douyin-sync-diagnostic', subscription)
    return () => {
      ipcRenderer.removeListener('video:douyin-sync-diagnostic', subscription)
    }
  },
  ...
  onTaskSchedulerChanged: (callback: () => void) => {
    const subscription = () => callback()
    ipcRenderer.on('scheduler:notif', subscription)
    ipcRenderer.on('scheduler:overdue', subscription)
    return () => {
      ipcRenderer.removeListener('scheduler:notif', subscription)
