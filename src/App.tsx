...
function App() {
  ...
  useEffect(() => {
    ...
    const runFtsQuery = async () => {
      ...
      if (searchQuery.startsWith('/task ')) {
        ...
        // Query tasks.db
        const tasksRes = await api.dbQuery('tasks', 'SELECT * FROM tasks WHERE title LIKE ? OR description LIKE ? LIMIT 3', [
        ...
        if (tasksRes?.success) {
          tasksRes.data.forEach((taskObj: any) => {
            results.push({
              type: 'tasks',
              title: taskObj.title,
              desc: t('app.search_desc_task', { priority: taskObj.priority, due_date: taskObj.due_date }),
              action: () => { setActiveScreen('tasks'); setTaskTab('list'); setSearchOpen(false) }
            })
          })
        }

        // Query notes.db (FTS5)
        const notesRes = await api.dbQuery('notes', 'SELECT * FROM notes WHERE title LIKE ? OR content LIKE ? LIMIT 3', [
          `%${searchQuery}%`, `%${searchQuery}%`
        ])
        if (notesRes?.success) {
          notesRes.data.forEach((n: any) => {
            results.push({
              ...
              action: () => { setActiveScreen('notes'); setSearchOpen(false) }
            })
          })
        }

        // Query books.db
        const booksRes = await api.dbQuery('books', 'SELECT * FROM books WHERE title LIKE ? OR author LIKE ? LIMIT 2', [
          `%${searchQuery}%`, `%${searchQuery}%`
        ])
        if (booksRes?.success) {
          booksRes.data.forEach((b: any) => {
            results.push({
              type: 'books',
              title: b.title,
              desc: t('app.search_desc_book', { author: b.author, progress: Math.round(b.progress) }),
              action: () => { setActiveScreen('books'); setSearchOpen(false) }
            })
          })
        }
      }
  ...
  const handleCreateTaskFromCmd = async (title: string) => {
    ...
    const res = await api.dbQuery('tasks', `
  ...
  const handleCreateNoteFromCmd = async (title: string) => {
    if (!api) return
    const res = await api.dbQuery('notes', `
    ...
    <div className="app-container">
      ...
      {searchOpen && (
        ...
        >
          ...
          >
            {/* Search Input */}
            <div style={{ display: 'flex', alignItems: 'center', padding: '14px 18px', borderBottom: '1px solid var(--color-border)', gap: '12px' }}>
              ...
              />
              <span className="kbd-shortcut" style={{ margin: 0 }}>Esc</span>
            </div>
            ...
            <div style={{ maxHeight: '360px', overflowY: 'auto', padding: '8px' }}>
              {searchResults.length === 0 ? (
                ...
                searchResults.map((result, idx) => (
                  ...
                  >
                    <div>
                      <strong style={{ fontSize: '13px', display: 'block', color: 'var(--text-main)' }}>{result.title}</strong>
                      <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{result.desc}</span>
                    </div>
                    <span className="pill" style={{ textTransform: 'uppercase', fontSize: '9px' }}>{result.type}</span>
                  </div>
                ))
              )}
            </div>
