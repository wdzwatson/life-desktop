...
import { Plus, Search, Play, Download, Settings, Trash2, ArrowUpDown, Layers, CheckSquare } from 'lucide-react'
...
export const Videos: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    // Load local videos
    const res = await api.dbQuery('videos', 'SELECT * FROM videos ORDER BY priority = "high" DESC, priority = "mid" DESC, favorite_time DESC')
  ...
  const handleQueueDownload = async () => {
    ...
    if (parsedData.isPlaylist) {
      ...
      setDownloadQueue(prev => [...prev, { title: parsedData.title, progress: 0, status: 'queued' }])
    ...
    <div style={{ animation: 'enter 0.15s ease both', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto' }}>
          {/* URL Parser Input */}
          <form onSubmit={handleParseUrl} className="card" style={{ display: 'flex', gap: '8px', padding: '12px' }}>
            <input 
              ...
              style={{ flexGrow: 1 }}
            />
            <button type="submit" className="btn primary">{t('videos.btn_parse_url')}</button>
          </form>

          {/* Local Library list */}
          <div className="card" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <strong style={{ fontSize: '13px' }}>{t('videos.downloaded_title')} ({filteredLocalVideos.length})</strong>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto' }}>
              {filteredLocalVideos.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: '12.5px', fontStyle: 'italic', margin: 'auto', padding: '36px' }}>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <label style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{t('videos.settings_label_concurrency')}: {concurrencyLimit}</label>
            <input 
              type="range"
              min="1"
              max="5"
          ...
          {/* Download Queue */}
          <div className="card" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto' }}>
            <strong style={{ fontSize: '13px' }}>{t('videos.download_queue')} ({downloadQueue.length})</strong>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto' }}>
              {downloadQueue.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', fontSize: '12px', fontStyle: 'italic', margin: 'auto', padding: '16px', textAlign: 'center' }}>
                ...
                downloadQueue.map((item, idx) => (
                  <div key={idx} style={{ padding: '8px', border: '1px solid var(--color-border)', borderRadius: '6px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '9.5px', color: 'var(--text-muted)', margin: '4px 0' }}>
                      <span>{item.status === 'queued' ? t('videos.status_queued') : t('videos.status_downloading')}</span>
                      <span>{item.progress}%</span>
                    </div>
                    <div style={{ height: '4px', backgroundColor: 'var(--color-border)', borderRadius: '2px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${item.progress}%`, backgroundColor: 'var(--color-accent)' }} />
                    </div>
                  </div>
                ))
              )}
      ...
      {parsedData && (
        <div style={{
          ...
          <div className="card" style={{ width: '520px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ flexGrow: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {parsedData.isPlaylist ? (
                parsedData.videos.map((v: any) => {
                  ...
                  return (
                    <label key={v.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px', border: '1px solid var(--color-border)', borderRadius: '6px', cursor: 'pointer' }}>
                      <input 
                        type="checkbox"
                        checked={isChecked}
                        onChange={() => {
                      ...
                      <span style={{ fontSize: '11px', color: 'var(--text-muted)', marginLeft: 'auto' }}>{v.duration}</span>
                    </label>
                  )
                })
              ) : (
                <div style={{ padding: '10px', border: '1px solid var(--color-border)', borderRadius: '6px' }}>
                  <div style={{ fontSize: '13px', fontWeight: 600 }}>{parsedData.title}</div>
                  <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{t('videos.duration_label')}: {parsedData.duration}</span>
                </div>
              )}
            </div>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              <button className="btn" onClick={() => setParsedData(null)}>{t('common.cancel')}</button>
              <button className="btn primary" onClick={handleQueueDownload}>{t('videos.btn_confirm_download')}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{
          ...
          <header style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <button className="btn sm" style={{ backgroundColor: '#222', borderColor: '#444', color: '#fff' }} onClick={() => setPlayingVideo(null)}>✕ {t('common.close')}</button>
              <span style={{ fontSize: '13px' }}>{t('videos.playing_label')}: {playingVideo.title}</span>
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <button className="btn sm" style={{ backgroundColor: '#222', borderColor: '#444', color: '#fff' }} onClick={() => { showToast(t('videos.toast_pip_enabled')); }}>
              ...
              <select 
                ...
                style={{ width: '80px', height: '28px', backgroundColor: '#222', color: '#fff', borderColor: '#444', fontSize: '12px' }}
              >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', height: '100%', minHeight: 0 }}>
            {/* Left Screen */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0A0A0A', position: 'relative' }}>
              <div style={{ fontSize: '48px', color: '#666' }}>🎬</div>
              <p style={{ color: '#aaa', fontSize: '13px', marginTop: '16px' }}>{t('videos.playback_canvas_tip', { speed: playbackSpeed })}</p>
              
              {/* Timeline control */}
              <div style={{ position: 'absolute', bottom: '24px', left: '24px', right: '24px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)' }}>12:15</span>
                <div style={{ height: '4px', backgroundColor: '#333', borderRadius: '2px', flexGrow: 1, overflow: 'hidden', cursor: 'pointer' }}>
                  <div style={{ height: '100%', width: '30%', backgroundColor: 'var(--color-accent)' }} />
                </div>
                <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)' }}>{playingVideo.duration || '42:15'}</span>
              </div>
            </div>

            {/* Right Subtitle Keyword Index Search Panel */}
            <aside style={{ backgroundColor: '#111', borderLeft: '1px solid #222', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <h4 style={{ fontSize: '11.5px', color: '#888', textTransform: 'uppercase' }}>{t('videos.subtitle_search_title')}</h4>
              <input 
                className="form-field"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', flexGrow: 1 }}>
                {subtitles.filter(s => s.text.includes(subtitlesQuery)).map((sub, idx) => (
                  <div 
                    key={idx}
                  ...
                  >
                    <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-accent)', fontWeight: 600, display: 'block', marginBottom: '2px' }}>
                      {sub.time}
                    </span>
