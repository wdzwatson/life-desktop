...
export const Tasks: React.FC = () => {
  ...
  const getPriorityLabel = (priority: string) => {
    switch (priority) {
      case 'high': return t('tasks.priority_high')
      case 'mid': return t('tasks.priority_mid')
      case 'low': return t('tasks.priority_low')
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case '待收集': return t('tasks.lane_inbox')
      case '待处理': return t('tasks.lane_todo')
      case '进行中': return t('tasks.lane_inprogress')
      case '待验收': return t('tasks.lane_review')
      case '已关闭': return t('tasks.lane_closed')
      case '已逾期': return t('common.overdue')
  ...
  const selectRule = (rule: any) => {
    ...
    setRuleWeekDays((rule.week_days || '').split(',').filter(Boolean).map((x: string) => parseInt(x)))
    setRuleMonthDays((rule.month_days || '').split(',').filter(Boolean).map((x: string) => parseInt(x)))
  ...
  const toggleTaskDone = async (task: any) => {
    ...
    // Update self
    await api.dbQuery('tasks', 'UPDATE tasks SET is_completed = ?, status = ?, progress = ? WHERE id = ?', [
      nextDone, nextStatus, nextDone ? 100 : 0, task.id
    ])

    // If task is completed and has children, ask or auto-close children
    if (nextDone) {
      await api.dbQuery('tasks', 'UPDATE tasks SET is_completed = 1, status = "已关闭", progress = 100 WHERE parent_id = ?', [task.id])
    }
  ...
  const handleSaveDetails = async () => {
    ...
    const query = `
      ...
      WHERE id = ?
    `
    const res = await api.dbQuery('tasks', query, [editDesc, editProgress, isCompleted, status, selectedTaskId])
    if (res?.success) {
  ...
  const handleSaveRule = async () => {
    ...
    if (selectedRuleId) {
      ...
      await api.dbQuery('tasks', query, [
        ruleName, ruleDesc, ruleFreq, ruleInterval, weekDaysStr, monthDaysStr, ruleCron, ruleHolidayPolicy, selectedRuleId
      ])
      ...
      `
      await api.dbQuery('tasks', query, [
        ruleName, ruleDesc, ruleFreq, ruleInterval, weekDaysStr, monthDaysStr, ruleCron, ruleHolidayPolicy
      ])
  ...
  const handleUseTemplate = async (template: any) => {
    ...
    const parentRes = await api.dbQuery('tasks', `
      INSERT INTO tasks (title, description, priority, status, due_date, is_completed, progress)
      VALUES (?, ?, 'mid', '待处理', ?, 0, 0)
  ...
  return (
    <div style={{ animation: 'enter 0.15s ease both', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="tabs" style={{ display: 'flex', gap: '4px', borderBottom: '1px solid var(--color-border)', marginBottom: '16px' }}>
        <button className={`tab ${taskTab === 'kanban' ? 'active' : ''}`} onClick={() => setTaskTab('kanban')}>{t('tasks.tab_kanban')}</button>
        <button className={`tab ${taskTab === 'list' ? 'active' : ''}`} onClick={() => setTaskTab('list')}>{t('tasks.tab_list')}</button>
        <button className={`tab ${taskTab === 'calendar' ? 'active' : ''}`} onClick={() => setTaskTab('calendar')}>{t('tasks.tab_calendar')}</button>
        <button className={`tab ${taskTab === 'recurring' ? 'active' : ''}`} onClick={() => setTaskTab('recurring')}>{t('tasks.tab_recurring')}</button>
        <button className={`tab ${taskTab === 'templates' ? 'active' : ''}`} onClick={() => setTaskTab('templates')}>{t('tasks.tab_templates')}</button>
        <button className={`tab ${taskTab === 'scheduled' ? 'active' : ''}`} onClick={() => setTaskTab('scheduled')}>{t('tasks.tab_scheduled')}</button>
      </div>

      <div style={{ flexGrow: 1, minHeight: 0 }}>
        {/* TAB: KANBAN BOARD */}
        {taskTab === 'kanban' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', height: '100%', overflowY: 'auto' }}>
            {[
              ...
              const laneTasks = tasks.filter(t => t.status === lane.dbVal || (lane.dbVal === '待处理' && t.status === '已逾期'))
              return (
                <div key={lane.key} style={{
                  ...
                  <div style={{ fontWeight: 'bold', fontSize: '13px', display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)' }}>
                    ...
                    {laneTasks.map(task => (
                      <div 
                        ...
                        style={{ 
                          padding: '12px', 
                          cursor: 'pointer',
                          borderColor: task.status === '已逾期' ? 'var(--color-danger)' : 'var(--color-border)',
                          boxShadow: task.status === '已逾期' ? '0 0 4px rgba(239, 68, 68, 0.15)' : 'var(--shadow-app)'
                        }}
                        onClick={() => {
                      ...
                      >
                        <h4 style={{ fontSize: '12.5px', fontWeight: 600, color: 'var(--text-main)' }}>
                          {task.status === '已逾期' && <span style={{ color: 'var(--color-danger)', marginRight: '4px' }}>[{t('common.overdue')}]</span>}
                          {task.title}
                        </h4>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
                          <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{task.due_date || t('tasks.due_date_not_set')}</span>
                          <span className={`pill ${task.priority === 'high' ? 'red' : task.priority === 'mid' ? 'yellow' : 'green'}`} style={{ fontSize: '9px', transform: 'scale(0.85)' }}>
        ...
        {taskTab === 'list' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '16px', height: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto' }}>
              {/* Quick Add Bar */}
              <form onSubmit={handleQuickAdd} style={{ display: 'flex', gap: '8px', padding: '4px' }}>
                <input 
                  ...
                  style={{ flexGrow: 1 }}
                />
                <select className="form-field" value={quickPriority} onChange={e => setQuickPriority(e.target.value)} style={{ width: '80px' }}>
                    ...
                    <div key={task.id} style={{ display: 'flex', flexDirection: 'column' }}>
                      <div 
                        style={{
                          ...
                          borderColor: isOverdue ? 'var(--color-danger)' : (isSelected ? 'var(--color-accent)' : 'var(--color-border)'),
                          cursor: 'pointer'
                        }}
                        onClick={() => {
                          setSelectedTaskId(task.id)
                      ...
                      >
                        <button 
                          ...
                          style={{ border: 'none', background: 'none', display: 'flex', alignItems: 'center' }}
                        >
                          {task.is_completed === 1 ? (
                            <Check size={16} color="var(--color-success)" />
                          ) : (
                            <Circle size={16} color={isOverdue ? 'var(--color-danger)' : 'var(--text-muted)'} />
                          )}
                        </button>
                        <span style={{ fontSize: '11px', color: isOverdue ? 'var(--color-danger)' : 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          {task.progress > 0 && task.progress < 100 && (
                            <div style={{ height: '3px', backgroundColor: 'var(--color-border)', borderRadius: '2px', marginTop: '4px', width: '80px' }}>
                              <div style={{ height: '100%', width: `${task.progress}%`, backgroundColor: 'var(--color-accent)' }} />
                            </div>
                          )}
                        </div>
                        <span className={`pill ${task.priority === 'high' ? 'red' : task.priority === 'mid' ? 'yellow' : 'green'}`} style={{ fontSize: '10px' }}>
                          {getPriorityLabel(task.priority)}
                        </span>
                        <button className="btn sm" onClick={(e) => { e.stopPropagation(); handleAddSubtask(task.id) }} title={t('tasks.add_subtask_tooltip')}>
                          ＋
                        </button>
                      </div>
                      ...
                      {children.map(child => (
                        <div 
                          ...
                          style={{
                            ...
                            backgroundColor: selectedTaskId === child.id ? 'rgba(59, 130, 246, 0.02)' : 'transparent',
                            cursor: 'pointer'
                          }}
                          onClick={() => {
                        ...
                        >
                          <button 
                            onClick={(e) => { e.stopPropagation(); toggleTaskDone(child) }}
                            style={{ border: 'none', background: 'none', display: 'flex', alignItems: 'center' }}
                          >
                          ...
                          <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{child.due_date}</span>
                          <span style={{ 
                            fontSize: '12.5px', 
                            textDecoration: child.is_completed === 1 ? 'line-through' : 'none',
                            color: child.is_completed === 1 ? 'var(--text-muted)' : 'var(--text-main)'
                          }}>{child.title}</span>
                          <span className={`pill ${child.priority === 'high' ? 'red' : child.priority === 'mid' ? 'yellow' : 'green'}`} style={{ fontSize: '9px', transform: 'scale(0.85)' }}>
            ...
            {/* Right details panel */}
            <aside className="card" style={{ display: 'flex', flexDirection: 'column', gap: '14px', height: '100%', overflowY: 'auto' }}>
              {activeTask ? (
                <>
                  <h3 style={{ fontSize: '15px', fontWeight: 800 }}>{t('tasks.details_title')}</h3>
                  <div>
                    <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.details_label_title')}</label>
                    <div style={{ fontSize: '14px', fontWeight: 700 }}>{activeTask.title}</div>
                  </div>
                  <div>
                    <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.details_label_desc')}</label>
                    <textarea 
                      className="form-field" 
                      rows={3} 
                      value={editDesc} 
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                      <span className="pill">{getPriorityLabel(activeTask.priority)} {t('tasks.details_priority_suffix')}</span>
                      <span className="pill">{t('tasks.details_due_prefix')} {activeTask.due_date || t('tasks.due_date_not_set')}</span>
                    </div>
                  </div>
                  ...
                  {!activeTask.parent_id && tasks.some(c => c.parent_id === activeTask.id) ? (
                    <div>
                      <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.details_subtask_progress')}</label>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ height: '6px', backgroundColor: 'var(--color-border)', borderRadius: '99px', flexGrow: 1, overflow: 'hidden' }}>
                          <div style={{ height: '100%', width: `${activeTask.progress}%`, backgroundColor: 'var(--color-accent)' }} />
                        </div>
                        <span style={{ fontSize: '11px', fontWeight: 'bold' }}>{activeTask.progress}%</span>
                      </div>
                      <p style={{ fontSize: '10.5px', color: 'var(--text-muted)', marginTop: '4px' }}>{t('tasks.details_subtask_progress_tip')}</p>
                    </div>
                  ) : (
                    <div>
                      <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.details_label_progress')}: {editProgress}%</label>
                      <input 
                        type="range" 
                        min="0" 
                        max="100" 
                  ...
                  <button className="btn primary" onClick={handleSaveDetails} style={{ marginTop: 'auto' }}>
                    {t('tasks.btn_save_changes')}
                  </button>
                </>
              ) : (
                <div style={{ textAlign: 'center', color: 'var(--text-muted)', margin: 'auto', fontStyle: 'italic', fontSize: '12px' }}>
        ...
        {taskTab === 'calendar' && (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px', height: '100%' }}>
            ...
            {/* Day View */}
            {calendarMode === 'day' && (
              <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: '10px', flexGrow: 1, overflowY: 'auto' }}>
                <div style={{ borderRight: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', gap: '40px', paddingRight: '8px', color: 'var(--text-muted)', fontSize: '11px', fontFamily: 'var(--font-mono)' }}>
                  <div>09:00</div>
                  <div>12:00</div>
                  <div>14:00</div>
                  <div>17:00</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <div style={{ padding: '8px 12px', backgroundColor: 'rgba(59, 130, 246, 0.08)', borderLeft: '4px solid var(--color-accent)', borderRadius: '4px' }}>
                    <strong style={{ fontSize: '13px', display: 'block' }}>{t('tasks.calendar_mock_weekly_planning')}</strong>
                    <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>09:00 - 10:00 · {t('tasks.calendar_label_recurring')}</span>
                  </div>
                  <div style={{ padding: '8px 12px', backgroundColor: 'rgba(245, 158, 11, 0.08)', borderLeft: '4px solid var(--color-warn)', borderRadius: '4px' }}>
                    <strong style={{ fontSize: '13px', display: 'block' }}>{t('tasks.calendar_mock_performance_tuning')}</strong>
                    <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>14:00 - 15:30 · {t('tasks.calendar_label_dev')}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {calendarMode === 'week' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px', flexGrow: 1, overflowY: 'auto' }}>
                {[
                  { day: t('tasks.calendar_day_mon'), tasks: [{ title: t('tasks.calendar_mock_weekly_planning'), time: '09:00' }] },
                  { day: t('tasks.calendar_day_tue'), tasks: [] },
                  { day: t('tasks.calendar_day_wed'), tasks: [{ title: t('tasks.calendar_mock_perf_tuning_short'), time: '14:00' }] },
                  { day: t('tasks.calendar_day_thu'), tasks: [{ title: t('tasks.calendar_mock_sync_flow'), time: '10:00' }] },
                  { day: t('tasks.calendar_day_fri'), tasks: [] },
                  { day: t('tasks.calendar_day_sat'), tasks: [{ title: t('tasks.calendar_mock_clear_clipboard'), time: '16:00' }] },
                  { day: t('tasks.calendar_day_sun'), tasks: [{ title: t('tasks.calendar_mock_daily_review'), time: '21:00' }] }
                ].map(col => (
                  <div key={col.day} style={{ border: '1px solid var(--color-border)', borderRadius: '6px', padding: '8px', display: 'flex', flexDirection: 'column', gap: '8px', backgroundColor: 'var(--bg-app)' }}>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', borderBottom: '1px solid var(--color-border)', paddingBottom: '4px', textAlign: 'center' }}>
                      {col.day}
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flexGrow: 1 }}>
                      {col.tasks.map((t, idx) => (
                        <div key={idx} style={{ padding: '6px', fontSize: '11px', backgroundColor: 'var(--bg-surface)', border: '1px solid var(--color-border)', borderRadius: '4px', borderLeft: '3px solid var(--color-accent)', whiteSpace: 'normal', wordBreak: 'break-all' }}>
                          <div style={{ fontWeight: 'bold' }}>{t.title}</div>
                          <div style={{ color: 'var(--text-muted)', fontSize: '9px', marginTop: '2px' }}>{t.time}</div>
                        </div>
                      ))}
                      {col.tasks.length === 0 && (
                        <div style={{ color: 'var(--text-muted)', fontSize: '10px', fontStyle: 'italic', textAlign: 'center', marginTop: '10px' }}>{t('tasks.calendar_no_schedule')}</div>
                      )}
                    </div>
                  </div>
                ))}
            ...
            {/* Month View */}
            {calendarMode === 'month' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gridTemplateRows: '30px repeat(5, 1fr)', gap: '6px', flexGrow: 1 }}>
                {(t('tasks.calendar_month_headers').split(',')).map((w, idx) => (
                  <div key={idx} style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--text-muted)', textAlign: 'center' }}>
                    {w}
                  </div>
                ))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '16px', height: '100%' }}>
            ...
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button className="btn sm primary" onClick={handleNewRule}>＋</button>
              </div>
                ...
                {rules.map(rule => (
                  <div 
                    ...
                    style={{
                      ...
                      border: '1px solid var(--color-border)',
                      borderRadius: '8px',
                      backgroundColor: selectedRuleId === rule.id ? 'rgba(59, 130, 246, 0.04)' : 'transparent',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px' }}>
                      {rule.frequency === 'daily' ? t('tasks.freq_daily') : rule.frequency === 'weekday' ? t('tasks.freq_weekday') : rule.frequency === 'weekly' ? t('tasks.freq_weekly') : rule.frequency === 'monthly' ? t('tasks.freq_monthly') : rule.frequency} · {rule.cron || t('tasks.no_cron')}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Right rule editor */}
            <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '14px', overflowY: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h3 style={{ fontSize: '15px', fontWeight: 800 }}>{t('tasks.config_rule_title')}</h3>
                {selectedRuleId && (
              ...
              <div>
                <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.rule_name_label')}</label>
                <input className="form-field" value={ruleName} onChange={e => setRuleName(e.target.value)} />
              </div>

              <div>
                <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.details_label_desc')}</label>
                <textarea className="form-field" rows={2} value={ruleDesc} onChange={e => setRuleDesc(e.target.value)} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px', gap: '10px' }}>
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.freq_label')}</label>
                  <select className="form-field" value={ruleFreq} onChange={e => setRuleFreq(e.target.value)}>
                ...
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.interval_label')}</label>
                  <input className="form-field" type="number" value={ruleInterval} onChange={e => setRuleInterval(parseInt(e.target.value))} />
                </div>
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.time_label')}</label>
                  <input className="form-field" type="time" value={ruleTime} onChange={e => setRuleTime(e.target.value)} />
                </div>
              </div>

              {ruleFreq === 'weekly' && (
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '6px' }}>{t('tasks.days_of_week_label')}</label>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    {[1, 2, 3, 4, 5, 6, 7].map(d => {
              ...
              {ruleFreq === 'cron' && (
                <div>
                  <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.freq_cron')}</label>
                  <input className="form-field" value={ruleCron} onChange={e => setRuleCron(e.target.value)} placeholder="0 22 * * *" />
              ...
              <div>
                <label style={{ fontSize: '11px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>{t('tasks.holiday_strategy_label')}</label>
                <select className="form-field" value={ruleHolidayPolicy} onChange={e => setRuleHolidayPolicy(e.target.value)}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {(t('tasks.future_triggers_list').split(',')
                  ).map(t => (
              ...
              <button className="btn primary" onClick={handleSaveRule} style={{ width: 'max-content', alignSelf: 'flex-end', marginTop: 'auto' }}>
        ...
        {/* TAB: TEMPLATES */}
        {taskTab === 'templates' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', overflowY: 'auto', height: '100%' }}>
            {templates.map(tpl => (
              <div key={tpl.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <ul style={{ paddingLeft: '18px', fontSize: '12px', color: 'var(--text-muted)', display: 'flex', flexDirection: 'column', gap: '4px', flexGrow: 1 }}>
                ...
                <button className="btn primary sm" onClick={() => handleUseTemplate(tpl)} style={{ width: 'max-content' }}>
        ...
        {taskTab === 'scheduled' && (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px', height: '100%', overflowY: 'auto' }}>
            <strong style={{ fontSize: '14px' }}>{t('tasks.scheduled_log_title')}</strong>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12.5px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--color-border)', textAlign: 'left', color: 'var(--text-muted)', fontSize: '11px' }}>
              ...
              <tbody>
                {scheduledLogs.map(log => (
                  <tr key={log.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    ...
                    <td style={{ padding: '10px 8px', fontFamily: 'var(--font-mono)' }}>{log.trigger}</td>
                    <td style={{ padding: '10px 8px' }}>
                      <span className={`pill ${log.status === '运行中' || log.status === 'Running' ? 'yellow' : (log.status === '已完成' || log.status === 'Finished' ? 'green' : 'blue')}`}>
                        {log.status}
                      </span>
                    </td>
                    <td style={{ padding: '10px 8px', fontFamily: 'var(--font-mono)' }}>{log.nextRun}</td>
                    <td style={{ padding: '10px 8px' }}>
                      <button className="btn sm" onClick={() => showToast(t('tasks.toast_action_triggered'))}>{t('tasks.btn_run_now')}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
