...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (api) {
      ...
      // 1. Fetch Today Tasks
      api.dbQuery('tasks', 'SELECT * FROM tasks WHERE due_date = ? OR status = "已逾期" LIMIT 3', [todayYMD]).then((res: any) => {
        if (res?.success) setTodayTasks(res.data)
      })

      // 2. Fetch Reading Book
      api.dbQuery('books', 'SELECT * FROM books WHERE status = "reading" LIMIT 1').then((res: any) => {
      ...
      // 3. Fetch Recent Note
      api.dbQuery('notes', 'SELECT * FROM notes ORDER BY updated_at DESC LIMIT 1').then((res: any) => {
        if (res?.success && res.data.length > 0) setRecentNote(res.data[0])
      })

      // 4. Fetch Recent Video
      api.dbQuery('videos', 'SELECT * FROM videos WHERE status = "Downloaded" ORDER BY id DESC LIMIT 1').then((res: any) => {
  ...
  const toggleTask = async (id: number, currentDone: boolean) => {
    ...
    if (api) {
      ...
      // Refresh list
      const todayYMD = new Date().toISOString().slice(0, 10)
      const res = await api.dbQuery('tasks', 'SELECT * FROM tasks WHERE due_date = ? OR status = "已逾期" LIMIT 3', [todayYMD])
      if (res?.success) setTodayTasks(res.data)
    }
  }
      ...
      <div className="grid-layout grid-3">
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          <div className="card-title">
            ...
            <button className="pill blue" onClick={() => navigateTo('tasks', 'list')} style={{ cursor: 'pointer', border: 'none' }}>
              {t('dashboard.manage_list')}
            </button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flexGrow: 1 }}>
            {todayTasks.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: '13px', fontStyle: 'italic', margin: 'auto' }}>
              ...
              todayTasks.map((task: any) => (
                ...
                >
                  {task.is_completed === 1 ? (
                    ...
                    <Circle size={18} color={task.status === '已逾期' ? 'var(--color-danger)' : 'var(--text-muted)'} />
                  )}
                  <div style={{ minWidth: 0, flexGrow: 1 }}>
                    ...
                    <span style={{ fontSize: '11px', color: task.status === '已逾期' ? 'var(--color-danger)' : 'var(--text-muted)' }}>
                    {task.status === '已逾期' ? t('common.overdue') : task.due_date}
                    </span>
                  </div>
                  <span className={`pill ${task.priority === 'high' ? 'red' : task.priority === 'mid' ? 'yellow' : 'green'}`} style={{ transform: 'scale(0.85)' }}>
                    {task.priority === 'high' ? t('tasks.priority_high') : task.priority === 'mid' ? t('tasks.priority_mid') : t('tasks.priority_low')}
                  </span>
                </div>
              ))
            )}
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          <div className="card-title">
            ...
            <button className="pill blue" onClick={() => navigateTo('books')} style={{ cursor: 'pointer', border: 'none' }}>
            ...
            {currentBook ? (
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
                ...
                <div style={{ minWidth: 0, width: '100%' }}>
                  <h3 style={{ fontSize: '14px', fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {currentBook.title}
                  </h3>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ height: '6px', backgroundColor: 'var(--color-border)', borderRadius: '99px', flexGrow: 1, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${currentBook.progress || 0}%`, backgroundColor: 'var(--color-accent)' }} />
                    </div>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          <div className="card-title">
            ...
            <button className="pill green" onClick={() => navigateTo('toolbox')} style={{ cursor: 'pointer', border: 'none' }}>
              {t('dashboard.pomodoro')}
            </button>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '100%', padding: '10px 8px' }}>
            <div>
              <div style={{ fontSize: '42px', fontWeight: 800, fontFamily: 'var(--font-mono)', letterSpacing: '-0.02em', color: 'var(--text-main)' }}>
                {timerLeft}
              </div>
              <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{t('dashboard.bind_task_hint')}</p>
            </div>
            <button 
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          <div className="card-title">
            ...
            <button className="pill blue" onClick={() => navigateTo('notes')} style={{ cursor: 'pointer', border: 'none' }}>
              {t('dashboard.open_notes')}
            </button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '8px' }}>
            {recentNote ? (
              <div style={{ padding: '8px 4px', cursor: 'pointer' }} onClick={() => navigateTo('notes')}>
                <h4 style={{ fontSize: '13px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          <div className="card-title">
            ...
            <button className="pill blue" onClick={() => navigateTo('videos')} style={{ cursor: 'pointer', border: 'none' }}>
            ...
            {recentVideo ? (
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <h4 style={{ fontSize: '13px', fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>
                    {recentVideo.title}
                  </h4>
                  <p style={{ color: 'var(--text-muted)', fontSize: '11px', marginTop: '2px' }}>
                    {recentVideo.source} · {recentVideo.duration || '00:00'} · {t('dashboard.downloaded')}
                  </p>
                </div>
              </div>
            ) : (
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', height: '100%', alignItems: 'center' }}>
            <div style={{ textAlign: 'center', padding: '6px', borderRight: '1px solid var(--color-border)' }}>
              <strong style={{ fontSize: '18px', display: 'block' }}>{stats.tasks}</strong>
              <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{t('dashboard.stats_tasks')}</span>
            </div>
            <div style={{ textAlign: 'center', padding: '6px' }}>
              <strong style={{ fontSize: '18px', display: 'block' }}>{stats.notes}</strong>
              <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{t('dashboard.stats_notes')}</span>
            </div>
            <div style={{ textAlign: 'center', padding: '6px', borderRight: '1px solid var(--color-border)', borderTop: '1px solid var(--color-border)', paddingTop: '8px' }}>
              <strong style={{ fontSize: '18px', display: 'block' }}>{stats.books}</strong>
              <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{t('dashboard.stats_books')}</span>
            </div>
            <div style={{ textAlign: 'center', padding: '6px', borderTop: '1px solid var(--color-border)', paddingTop: '8px' }}>
              <strong style={{ fontSize: '18px', display: 'block' }}>{stats.videos}</strong>
              <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{t('dashboard.stats_videos')}</span>
            </div>
          </div>
        </div>
      </div>
