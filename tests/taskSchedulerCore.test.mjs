...
test('scheduler writes the next day recurring task and its subtasks at midnight', () => {
  ...
  try {
    ...
    assert.deepEqual(db.prepare('SELECT due_date, due_time, instance_key FROM tasks WHERE parent_id IS NULL').all(), [{ due_date: '2026-07-22', due_time: '09:00', instance_key: '2026-07-22T09:00' }])
...
test('scheduler creates today task when the app starts after midnight', () => {
  ...
  try {
    ...
    assert.equal(result.generatedTasks.length, 1)
    assert.deepEqual(
      db.prepare('SELECT due_date, due_time, instance_key FROM tasks WHERE parent_id IS NULL').all(),
      [{ due_date: '2026-07-22', due_time: '09:00', instance_key: '2026-07-22T09:00' }],
    )
    db.close()
  } finally { rmSync(dir, { recursive: true, force: true }) }
})
