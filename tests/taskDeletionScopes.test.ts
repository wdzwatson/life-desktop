...
test('recurring task deletion preserves completed history while removing the selected scope', () => {
  ...
  assert.match(tasksView, /deletionScope === 'delete-all-repeat'/)
  assert.match(tasksView, /const deleteAllRecurringTaskTrees/)
  assert.match(tasksView, /SELECT id FROM tasks WHERE recur_rule_id = \? AND parent_id IS NULL/)
