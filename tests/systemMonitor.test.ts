...
import {
  normalizeDarwinProcessName,
  parseDarwinNetworkOutput,
  parseWindowsNetworkOutput,
} from '../electron/systemMonitor'
...
const componentSource = readFileSync(
  join(process.cwd(), 'src', 'components', 'SystemMonitor.tsx'),
  'utf8',
)
const monitorStyles = readFileSync(
  join(process.cwd(), 'src', 'components', 'SystemMonitor.css'),
  'utf8',
)
...
test('darwin network parsing follows netstat column headers and aggregates interface rows', () => {
  const counters = parseDarwinNetworkOutput(
...
test('darwin process paths are presented as application or executable names', () => {
  assert.equal(
    normalizeDarwinProcessName(
  ...
  assert.equal(normalizeDarwinProcessName('/usr/libexec/trustd'), 'trustd')
  assert.equal(normalizeDarwinProcessName('kernel_task'), 'kernel_task')
})

test('windows overview network parsing uses the first byte counter row without localized labels', () => {
  const counters = parseWindowsNetworkOutput(
...
test('desktop note reserves remaining height for tasks and bounds the resource popover', () => {
  ...
  assert.match(
    noteStyles,
    /\.desktop-task-note__content \{[\s\S]*flex: 1 1 auto;[\s\S]*min-height: 0;/,
  )
  assert.match(
    monitorStyles,
    /\.system-monitor--note \.system-monitor__rows \{[\s\S]*calc\(100vh - 190px\)/,
  )
  assert.match(
    monitorStyles,
    /\.system-monitor--note \.system-monitor__popover \{[\s\S]*bottom: calc\(100% \+ 7px\)/,
  )
  assert.match(noteStyles, /\.desktop-task-note__empty,[\s\S]*flex: 1 1 auto;/)
})
