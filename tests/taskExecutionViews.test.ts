...
test('list includes pending instances while kanban stays scoped to the execution task set', () => {
  ...
  assert.match(tasksView, /task\.status === TASK_STATUS\.closed/)
  assert.match(tasksView, /const \[dueDateFrom, setDueDateFrom\] = useState\(''\)/)
  assert.match(tasksView, /if \(!task\.due_date\) return !dueDateFrom && !dueDateTo/)
  assert.match(tasksView, /value=\{dueDateFrom[\s\S]*?onChange=[\s\S]*?clearable/)
